...
export default function Cartpage() {
    ...
    const { ho_ten_bn, sdt_bn, dia_chi, tinh_thanh, quan_huyen, phuong_xa, ghi_chu,tong_tien,ngay_sinh,gioi_tinh } = state;
    ...
    const handlePayment = (e) =>{
        ...
        if(window.confirm("Xác nhận lại thông tin dịch vụ , xác nhận đặt lịch ?")){
            e.preventDefault();
            const paymentMethod = selectedPayment; // Lấy giá trị từ state
    ...
    localStorage.setItem("voucher_sale", JSON.stringify(NovoucherData));

    const handleSelectCoupon = (coupon) => {
        // Tạo object chứa thông tin coupon
        ...
        var x = JSON.parse(localStorage.getItem("voucher_sale"));

        if(x.coupon_name !== coupon.coupon_name){
            // Ghi đè nếu voucher đã tồn tại
        localStorage.setItem("voucher_sale", JSON.stringify(voucherData));
        LoadData();
        }

        else if(x.coupon_name === coupon.coupon_name){
        localStorage.setItem("voucher_sale", JSON.stringify(NovoucherData));
        const radioButton = document.querySelector(`input[type="radio"][name="coupon-select"][value="${coupon.coupon_name}"]`);
            if (radioButton) {
            radioButton.style.backgroundColor = "initial"; // Đặt màu về mặc định
            }
        LoadData();
        }
